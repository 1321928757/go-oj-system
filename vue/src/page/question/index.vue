    ...
    <div class="sort-list">
      <div>
        ...
        <span
          ...
          :key="item.id"
          @click="getProblem(item.id, item.name)"
          :class="actSort == item.id ? 'act' : ''"
          >{{ item.name }}</span
        >
      </div>
      <el-input
...
// 获取问题列表
const getProblem = (sortId: number | null, sortName: String | null) => {
  ...
  api
    ...
    .then((res) => {
      ...
      if (res && res.data && res.data.code == 200) {
        quesList.value = res.data.data.page_list;
...
const getSortList = () => {
  api
    ...
    .then((res) => {
      if (res && res.data && res.data.code == 200) {
        sortList.value = res.data.data.page_list;
      }
    });
};
getSortList();
